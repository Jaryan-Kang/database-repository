...
def balanced(m):
    ...
    def torque(arm):
        """Calculate the torque applied by an arm."""
        return length(arm) * total_mass(end(arm))

    def total_torque(mobile):
        """Calculate the total torque applied by a mobile."""
        left_arm, right_arm = left(mobile), right(mobile)
        left_torque = torque(left_arm) if is_planet(end(left_arm)) else total_torque(end(left_arm))
        right_torque = torque(right_arm) if is_planet(end(right_arm)) else total_torque(end(right_arm))
        return left_torque == right_torque

    if is_planet(m):
        return True

    return total_torque(m) and balanced(end(left(m))) and balanced(end(right(m)))
