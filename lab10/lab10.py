...
def eval_and(expressions):
    """
    Evaluate 'and' expressions in a Scheme-like linked list structure.
    If all expressions evaluate to a truthy value, return the last truthy value.
    If any expression evaluates to a falsy value, return that value.
    Returns True if the list is empty.
    """
    current = expressions
    last_truthy = scheme_t  

    while current != nil:
        value = calc_eval(current.first) if isinstance(current.first, Pair) else current.first
        if not value:
            return value
        last_truthy = value
...
def eval_define(expressions):
    ...
    2
    """
    diyi = expressions.first
    r = calc_eval(expressions.rest.first)
    bindings[diyi] = r
    return diyi
